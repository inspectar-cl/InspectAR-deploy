...
import { LatestSensors, type SensorRow } from '@/components/dashboard/overview/latest-sensors';


function mockHistory24h(now = new Date(), n = 12, base = 40, spread = 6, unit = 'L/s') {
  return Array.from({ length: n }).map((_, i) => ({
    ts: dayjs(now).subtract((n - i) * 2, 'hour').toDate(),
    value: Number((base + (Math.random() - 0.5) * spread).toFixed(2)),
    unit,
  }));
}
